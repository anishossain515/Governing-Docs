    ...
    <nav className='py-5'>
        <Container>
            ...
            {/* list */}
             <div className=' hidden lg:flex items-center justify-center gap-4 text-accent'>
               {
                 navItems.map(({label,link,i})=>(
                   <a href={link} key={i} className=' hover:text-primary transition-colors duration-150 hover:underline '>
                     {label}
                   </a>
                 ))
               }
             ...
             <svg onClick={Toggle} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="lg:hidden block size-6 sm:size-8">
